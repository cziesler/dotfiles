...
$start = "\"\\\"\\\\<\${cmd}\\\\>\\\"\"" ;
#$end = "\"[;}\\]\]\\|[^\\\\]\$\"" ;
# Take out } as end marker - screws up code like this:
#      set_clock_groups -async \
#        -group {clk10 clk10div2} \
#        -group {clk99 clk99div2}
#  (and -edges {...})
$end = "\"[;\\]\]\\|[^\\\\]\$\"" ;
# The result of above is to end on:
#   ;
#   ]
#   unescaped newline
...
print STDERR "initial procfiles is @procfiles\n" if ($debug);
foreach $procdir (@procdirs) {
  if (-d $procdir && -r $procdir) {
    foreach $procfile (<$procdir/*>) {
      if (-f $procfile) {
        push(@procfiles,$procfile);
      }
    }
  } else {
...
print STDERR "procfiles is now @procfiles\n" if ($debug);
...
";
foreach $procfile (@procfiles) { 
  print "\"    procedures file $procfile\n";
}

print "\n\n";

print "
if ! exists(\"${prefix}_syntax_enable_eval\")
  let ${prefix}_syntax_enable_eval = \"1\"
endif
...
foreach $procfile (@procfiles) {
  print STDERR "Main loop opening proc file $procfile\n" if ($verbose);
  $cmd = "";
  $items = "";
  open(PROC,"< $procfile") || die "Error: Cannot open file $procfile\n";
  while (<PROC>) {
    ...
    if (/^\s*proc\s+(\S+)/) {
      # save previous
      if ($cmd) {
        print STDERR "Found new cmd $1, saving cmd $cmd\n" if ($verbose);
    ...
    if (/^\s*&?define_proc_attributes\s+(\S+)/) {
      ...
      &save_command($cmd,$items,$start,$end,$contains,$group,$link,""); 
    } elsif (/^\s*parse_options\s+(\S+)/ && $1 !~ /^\$/) {
      # found - grab command and init brace counting vars
      $cmd = $1;
      print STDERR "Found parse_options for $cmd\n" if ($verbose);

      # pull until unescaped end of line 
      while(/\\?$/) {
        $_ .= <PROC>;
      }
      print STDERR "parse_options line is: $_\n" if ($verbose);

      #@options = ($_ =~ m/(\{[^}*]\})/g);
      @options = ($_ =~ m/({.*})/g);
      #print STDERR "atoptions: \"@options\"\n";
      foreach $option (@options) {
        #$option =~ s/^\s+//;
        #$option =~ s/[{}]//;
        ($thisoption) = ($option =~ /(-\S+)/);
        $items = $items . "$thisoption ";
        if ($verbose) {
          print STDERR "option: \"$option\"\n";
          print STDERR "thisoption: \"$thisoption\"\n";
...
if ($cmd_abv) {
  ...
  foreach $abv (split(/ /,$cmd_abvs)) {
    ...
    #print STDERR "abv is $abv\n";
    if ($use_new_cmd_abv) {
      $abvstart = "\"\\<" . $abv; # "\<create_cl[ock]
      $abvstart =~ s/\[\]//;      # remove [] if it exists to avoid subst below
      $abvstart =~ s/\[/\\%\[/; # "\<create_cl\%[ock]
      $abvstart .= "\\>\"";
    } else {
      $abvstart = "\"\\<\\(";
      chop($abv); # get rid of ]
...
sub print_command {
  ...
  if ($cmd_abv) {
    ...
    if ($abvstart) {
      ...
      push(@cmd_abv_lines,"\" $cmd abbreviation: $commands{$cmd}{abv}\n");
...
#      push(@cmd_abv_eval_lines,"\" $cmd abbreviation: $commands{$cmd}{abv}\n");
...
sub process_options {
  ...
  die unless GetOptions(
    ...
    "cmd_abv!" => \$cmd_abv,
    "eval=s" => \@evals,
    "use_new_cmd_abv!" => \$use_new_cmd_abv,
    );
...
sub show_help {
 print "Syntax:
  ...
  -(no)use_new_cmd_abv Use the new command abbreviation code
